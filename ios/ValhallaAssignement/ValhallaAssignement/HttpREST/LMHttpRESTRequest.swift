//
//  LMHttpRESTRequest.swift
//  LMHttpRESTRequest_CoreLib_iOS
//
//  Created by John Freidrich Cinco on 7/23/18.
 ...
 */
open class LMHttpRESTRequest {
    private final let TAG:String = "LMHttpRESTRequest"
    ...
    {
        // DebugLog(tag:description(), info: String(format:"Creating %@ request for %@", method, url.absoluteString))
    ...
    internal func requestComplete(withData data:Data?, response:URLResponse?, error:Error?)
    {
        ...
        if (error != nil)
        {
            // ErrorLog(tag:description(), info: String(format: "Error: %@", error!.localizedDescription))
            // ErrorLog(tag:description(), info: String(format: "Debug desc: %@", error.debugDescription))
        }
        else
        {
            httpResponse = (response as? HTTPURLResponse)!
            // DebugLog(tag:description(), info: String(format: "Http Request Completed: %@", (response?.url?.absoluteString)!))
            // DebugLog(tag:description(), info: String(format: "Status: %d", httpResponse!.statusCode))
        }
    ...
    public func addHeader(value:String, forHTTPHeaderField field:String)
    {
        if (urlRequest != nil)
        {
            // DebugLog(tag:description(), info: String(format: "Setting %@ = %@", field, value))
    ...
    private func processUrlParams() {
        if (nil != urlParams) {
            ...
            for i in 0..<urlParams!.count { //var param:UrlParam in urlParams! {
                ...
                // DebugLog(tag:description(), info:"param \(param.name!)=\(param.value!)")
                let amp:String = (index > 0) ? "&" : ""
                urlParamStr = urlParamStr + amp + param.name! + "=" + param.value!
                index += 1
            }
            
            // DebugLog(tag:description(), info: urlParamStr)
            urlString = urlString! + urlParamStr
            
            // DebugLog(tag:description(), info: urlString!)
    ...
    public func execute()
    {
        // DebugLog(tag:description(), info: String(format:"Sending %@ request.", (urlRequest?.httpMethod!)!))
        processUrlParams()
